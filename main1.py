...
def add_vignette_border_and_auto_text(image_path, output_path, corner_radius=30, 
    ...
    with Image.open(image_path) as img:
        ...
        # 3. 定义边框宽度（按图片比例自动调整，避免边框过大/过小）
        border_width = max(30, int(min(width, height) * 0.1))  # 最小30px，最大为图片短边的5%
        new_width = width + 2 * border_width
        new_height = height + 2 * border_width
        
        # 4. 创建新画布并先放大原图填充整个画布（作为虚化背景）
        # 将原图放大并模糊作为背景
        background = img.copy()
        background = background.resize((new_width, new_height), Image.Resampling.LANCZOS)
        
        # 5. 对背景进行高斯模糊（虚化效果）
        blur_radius = max(20, border_width // 3)  # 模糊半径
